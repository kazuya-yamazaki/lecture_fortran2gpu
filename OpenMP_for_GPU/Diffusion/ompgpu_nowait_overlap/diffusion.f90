...
contains
  ...
  double precision function diffusion3d_halo(nprocs, rank, nx, ny, nz, dx, dy, dz, dt, kappa, f, fn)
    ...
    k = 1
!    !$omp target nowait depend(in:f) depend(out:fn(1:nx,1:ny,1))
    !$omp target
    !$omp loop     &
    !$omp& collapse(2) 
    do j = 1, ny
    ...
    k = nz
!    !$omp target nowait depend(in:f) depend(out:fn(1:nx,1:ny,nz))
    !$omp target
    !$omp loop   &
    !$omp& collapse(2) 
    do j = 1, ny
...
!    !$omp target nowait depend(in:f(1:nx,1:ny,2:nz-1)) depend(out:fn)
    !$omp target nowait 
    !$omp loop    &
    !$omp& collapse(3) 
    do k = 2, nz-1

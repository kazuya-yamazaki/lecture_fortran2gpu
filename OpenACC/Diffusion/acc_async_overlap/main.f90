program main
  ...
  do i = 0, nprocs-1
     ...
     write(*,'(A5,I2,A14,A12,A10,I2)') "Rank ", rank, ": hostname = ", hostname
  end do
  ...
  do icnt = 0, nt-1
     ...
     if (mod(icnt,2) == 0) then
        ...
        flop = flop + diffusion3d_halo(nprocs, rank, nx, ny, nz, dx, dy, dz, dt, kappa, fn, f)
        !$acc wait(0)
        !$acc wait(1)
        ...
        call MPI_Waitall(4,ireq,istat,ierr)
        !$acc wait(2)
        flop = flop + diffusion3d(nprocs, rank, nx, ny, nz, dx, dy, dz, dt, kappa, fn, f)
     endif

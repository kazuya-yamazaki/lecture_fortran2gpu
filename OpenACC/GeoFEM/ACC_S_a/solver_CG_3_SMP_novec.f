...
!$acc data
!$acc& copyin(NOD_EXPORT(istart:iend))
!$acc& present(AL,AU,ALU,D,B,X,WW,WS,WR)
!$acc& present(INL,IAL,INU,IAU)
...
!$acc end kernels
      ...
      BNRM20= 0.d0
!$acc kernels
!$acc loop independent reduction(+:BNRM20)
      do i= 1, 3*N
        BNRM20= BNRM20+B(i)**2
      enddo
!$acc end kernels
...
!$acc kernels
!$acc loop independent reduction(+:RHO0)
      do i= 1, 3*N
...
!C===
      C10= 0.d0
!$acc kernels
!$acc loop independent reduction(+:C10)
      do i= 1, 3*N
...
!$acc end kernels
      ...
      DNRM20= 0.d0
!$acc kernels
!$acc loop independent reduction(+:DNRM20)
      do i= 1, 3*N
